...
| RAlt + 其它键（如左右方向键） | 忽略，不触发、不打断听写（保持现状） |

### 2.3 改动点

#### 2.3.1 fcitx5 插件（openless.cpp）

- 在 `dictationTriggerHeld_` 分支中，额外识别 `RCtrl`（X11 keysym `0xffe3`）：
...
- 移动端顶部栏：标题 + 设置按钮。
- 底部导航：`概览`、`历史`、`风格`（弹出 StyleSheet：润色模式/风格市场）、`更多`（弹出 MoreSheet：词汇/翻译/划词追问/设置）、`设置`。
- 桌面侧栏在移动端隐藏。
...
| 页面 | 当前内容 | 移动端相关度 |
| --- | --- | --- |
| 概览 Overview | ASR/LLM 提供商状态、今日指标、周期指标、最近识别 | 中：远程听写用户主要关心状态与最近记录 |
| 历史 History | 搜索、模式筛选、详情、复制、删除、清空、重新润色、重新转录 | 高 |
| 词汇 Vocab | 词典管理 | 低（远程听写场景） |
| 润色模式 Style | 风格包/模式配置 | 低 |
| 风格市场 Marketplace | 安装/管理风格包 | 低 |
| 翻译 Translation | 翻译目标语言等 | 低（除非双模式中的“清洗”要复用翻译设置） |
| 划词追问 SelectionAsk | 选中文本 QA | 低 |
...
1. **移动端首页改为「远程听写」状态页**：只显示 LAN 服务器运行状态、保活状态、最近一次自检、当前手机 IP/端口，替代现有概览。
2. **隐藏与远程听写无关的桌面页**：词汇、风格市场、划词追问、翻译默认从移动端导航移除，仅保留历史 + 设置。
3. **设置按 Android 场景裁剪**：只保留「权限与保活」「服务（ASR/LLM）」「关于」，隐藏本地模型/主题/语言/高级等。
4. **保活状态整合进首页**：单卡片显示通知保活、单像素保活、电池优化限制、最近错误，并提供一键修复。
5. **Onboarding 精简**：合并悬浮窗权限+配置为一步，或把 ASR/LLM 配置延后到首次使用时引导。

> 本轮只列清单，不实施；等用户选定精简范围后再编码。
...
- [ ] 双模式快捷键映射是否如上（RAlt=raw，RAlt+RCtrl=clean）。
- [ ] 移动端精简范围（3.2 中 1–5 的选择）。
