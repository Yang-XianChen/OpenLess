...
export const zhCN = {
  ...
  style: {
    ...
    pack: {
      ...
      selectionListDesc: '用于无需 ASR 的已选文字：单纯语法、清晰度和格式润色。可为它单独选择风格与 Prompt。',
  ...
  settings: {
    ...
    recording: {
      ...
      silenceAutoStopDesc: '仅切换模式生效。检测到语音后，连续静音达到所选时长即自动结束并提交；一直没说话则 10 秒后取消。默认关闭；第二次按键停止和 Esc 取消仍然有效。',
    ...
    channels: {
      ...
      errGeneric: '失败',
      dragHint: '按住拖动可调整优先级',
      orderHint: '拖动排序，最上面的优先使用；关掉的渠道会自动排到末尾。',
      empty: '还没有渠道。点下面的「添加渠道」新建一个。',
      add: '添加渠道',
      edit: '编辑',
      createTitle: '添加渠道',
      editTitle: '编辑渠道',
      ...
      hoursAgo: '{{count}} 小时前',
      daysAgo: '{{count}} 天前',
      localEngineModelHint: '本地引擎的模型下载与切换在「高级 → 本地模型」里。',
    },
    providers: {
      ...
      pipelineIsolationNotice: '两种模式使用完全独立的凭据配置。切换模式不会删除另一套配置，只是暂时停用；切回即恢复。',
      presets: {
        ark: 'ARK（火山方舟）',
        deepseek: 'DeepSeek',
        siliconflow: '硅基流动',
      ...
      asrAdvancedChunkHint: '0 = 不分片，整段发送；按片段多次请求，适合长录音或服务端单次请求时长受限。',
      asrAdvancedEnableItnLabel: '数字归一化 (enable_itn)',
      asrAdvancedEnableItnHint: '把口语数字/单位归一化为阿拉伯数字（如“二零二六年”→“2026年”）。关闭后保留原始文字。',
      ...
      volcengineApiKeyNote: '使用新版语音控制台创建的 API Key 鉴权，无需 APP ID。API Key 在语音控制台「API Key 管理」中创建：console.volcengine.com/speech/new/setting/apikeys。Resource ID 默认使用 volc.seedasr.sauc.duration。',
      xfyunAppIdLabel: 'AppID',
      xfyunApiKeyLabel: 'API Key',
      xfyunNote: '在讯飞开放平台「实时语音转写」服务页获取 AppID 与 API Key。音频为 16kHz/16bit/单声道 PCM；标准版接口暂不支持热词参数（可在讯飞控制台配置个性化热词），语种默认中文普通话。',
    ...
    permissions: {
      ...
      androidShizukuHint: '可选功能，在部分机型无法手动开启无障碍时尽力恢复；无法完全消除跨应用竞态。设备重启后可能需要重新启动 Shizuku。',
      androidShizukuOpenApp: '打开 Shizuku',
      androidShizukuRequestPermission: '请求授权',
      androidShizukuRecover: '恢复无障碍服务',
      androidShizukuRecoverConfirm: '是否通过 Shizuku 尝试重新启用 OpenLess 无障碍服务？写入时会合并当时已启用的服务。若全局开关为关闭，启用后可能同时启动列表中已登记的其他无障碍服务。',
      androidShizukuYes: '是',
      androidShizukuNo: '否',
      ...
      androidShizukuMessages: {
        ...
        manual_required: '全局开关关闭且列表中已有其他无障碍服务时，无法安全自动恢复。请前往系统设置手动操作。',
      ...
      androidNotificationKeepaliveLabel: '通知保活',
      androidNotificationKeepaliveEnabled: '常驻“远程听写待命”通知以保持前台服务',
      androidNotificationKeepaliveHint: '建议用于远程听写；服务被系统重建时会自动恢复 LAN 服务，失败时通过通知提示解决方案。',
      androidKeepaliveStatusLabel: '远程听写保活状态',
      androidKeepaliveStatusHint: '最近自检：{{lastCheckAt}}',
      androidKeepaliveDiagHint: '服务已启动 {{restartCount}} 次 · 上次启动 {{lastServiceStartAt}} · 上次退出 {{lastProcessDeathAt}}',
      androidKeepaliveRestart: '重启保活服务',
      androidKeepaliveSelfTest: '运行保活自测',
      androidKeepaliveSelfTestKill: '进程级重启自测',
      androidKeepaliveSelfTestKillConfirm: '将立即结束本应用进程，由系统重启服务并验证 LAN 恢复，当前窗口会短暂消失，请继续？',
      androidKeepaliveSelfTestRecovered: '进程级自测已通过：进程被杀后 LAN 服务自动恢复（{{recoveredAt}}）',
      androidKeepaliveOpenNotification: '开启通知权限',
      androidKeepaliveOpenBattery: '电池设置',
      androidKeepaliveRunning: '运行中',
      androidKeepaliveLanOnly: '仅 LAN 服务运行',
      androidKeepaliveStopped: '已停止 / 异常',
  ...
  localAsr: {
    ...
    sherpaModelWhisperLargeV3Desc: '开源多语通用里效果最好的 Whisper 档，质量高、体积大，适合高质量转写。',
    sherpaModelQwen3: 'Qwen3-ASR 0.6B INT8',
    sherpaModelQwen3Desc: '转换后的 sherpa-onnx Qwen3-ASR 模型，支持多语言识别与更强的长上下文能力。',
    modelSelectTitle: '模型下载与管理',
    modelSelectDesc: '下载的模型会自动出现在上方「ASR 语音转写」的本地供应商里，选择即可使用；点击「＋ 下载新模型」获取更多模型。',
    ...
    downloadDialogAlreadyHave: '该模型已下载，可直接使用',
    downloadDialogDesc: '选择左侧模型后点击「开始下载」。下载完成后，到「服务 → AI 提供商 → ASR 语音转写」选择本地模型供应商即可使用。',
    detailRepo: '模型仓库',
    hfDownloads: '下载量',
    hfLikes: '收藏数',
    hfDescription: '模型简介',
