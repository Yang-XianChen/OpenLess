...
export const zhTW: typeof zhCN = {
  ...
  settings: {
    ...
    providers: {
      ...
      pipelineIsolationNotice: '兩種模式使用完全獨立的憑證設定。切換模式不會刪除另一套設定，只是暫時停用；切回即恢復。',
      presets: {
        ark: 'ARK（火山方舟）',
        deepseek: 'DeepSeek',
        siliconflow: '硅基流動',
      ...
      volcengineApiKeyNote: '使用新版語音控制台建立的 API Key 鑑權，無需 APP ID。API Key 可在語音控制台「API Key 管理」建立：console.volcengine.com/speech/new/setting/apikeys。Resource ID 預設使用 volc.seedasr.sauc.duration。',
      xfyunAppIdLabel: 'AppID',
      xfyunApiKeyLabel: 'API Key',
      xfyunNote: '在訊飛開放平台「即時語音轉寫」服務頁取得 AppID 與 API Key。音訊為 16kHz/16bit/單聲道 PCM；標準版介面暫不支援熱詞參數（可在訊飛控制台設定個人化熱詞），語種預設中文普通話。',
    ...
    permissions: {
      ...
      androidShizukuHint: '可選功能，在部分機型無法手動開啟無障礙時盡力恢復；無法完全消除跨應用競態。裝置重啟後可能需要重新啟動 Shizuku。',
      ...
      androidShizukuMessages: {
        ...
        manual_required: '全域開關關閉且清單中已有其他無障礙服務時，無法安全自動恢復。請前往系統設定手動操作。',
      ...
      androidSinglePixelKeepaliveLabel: '單像素懸浮窗保活',
      androidSinglePixelKeepaliveEnabled: '啟用 1×1 不可互動懸浮窗保持程式存活',
      androidSinglePixelKeepaliveHint: '需授予懸浮窗權限；僅用於保活，不顯示任何內容、不可互動。',
      androidNotificationKeepaliveLabel: '通知保活',
      androidNotificationKeepaliveEnabled: '常駐「遠端聽寫待命」通知以保持前景服務',
      androidNotificationKeepaliveHint: '建議用於遠端聽寫；服務被系統重建時會自動恢復 LAN 服務，失敗時透過通知提示解決方案。',
      androidKeepaliveStatusLabel: '遠端聽寫保活狀態',
      androidKeepaliveStatusHint: '最近自檢：{{lastCheckAt}}',
      androidKeepaliveDiagHint: '服務已啟動 {{restartCount}} 次 · 上次啟動 {{lastServiceStartAt}} · 上次退出 {{lastProcessDeathAt}}',
      androidKeepaliveRestart: '重新啟動保活服務',
      androidKeepaliveSelfTest: '執行保活自測',
      androidKeepaliveSelfTestKill: '進程級重啟自測',
      androidKeepaliveSelfTestKillConfirm: '將立即結束本應用進程，由系統重啟服務並驗證 LAN 恢復，當前視窗會短暫消失，請繼續？',
      androidKeepaliveSelfTestRecovered: '進程級自測已通過：進程被殺後 LAN 服務自動恢復（{{recoveredAt}}）',
      androidKeepaliveOpenNotification: '開啟通知權限',
      androidKeepaliveOpenBattery: '電池設定',
      androidKeepaliveRunning: '運行中',
      androidKeepaliveLanOnly: '僅 LAN 服務運行',
      androidKeepaliveStopped: '已停止 / 異常',
      ...
      androidOverlayTrigger: { background: '退到背景', keyboard: '鍵盤彈出時', always: '常駐', off: '關閉' },
      androidOverlayTriggerHint: { background: '省電', keyboard: '此模式已暫緩，既有設定會改回退到背景。', always: '一直佔屏', off: '完全關閉聽寫懸浮按鈕；單像素保活懸浮窗仍可獨立開啟。' },
  ...
  localAsr: {
    ...
    modelSelectDesc: '下載的模型會自動出現在上方「ASR 語音轉寫」的本地供應商裡，選擇即可使用；點擊「＋ 下載新模型」取得更多模型。',
