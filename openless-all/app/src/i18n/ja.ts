...
export const ja: typeof zhCN = {
  ...
  settings: {
    ...
    permissions: {
      ...
      androidSinglePixelKeepaliveLabel: '単画素オーバーレイで常駐',
      androidSinglePixelKeepaliveEnabled: '1×1 の非インタラクティブなオーバーレイを有効にしてプロセスを維持',
      androidSinglePixelKeepaliveHint: 'オーバーレイ権限が必要です。表示されず操作もできません。常駐維持のみに使われます。',
      androidNotificationKeepaliveLabel: '通知による常駐維持',
      androidNotificationKeepaliveEnabled: '「リモート入力待機中」通知を常駐させてフォアグラウンドサービスを維持',
      androidNotificationKeepaliveHint: 'リモート入力での使用を推奨。サービス再起動時にLANサーバーを自動復旧し、失敗時は通知で対処方法を案内します。',
      androidKeepaliveStatusLabel: 'リモート入力の常駐ステータス',
      androidKeepaliveStatusHint: '最終チェック: {{lastCheckAt}}',
      androidKeepaliveDiagHint: 'サービス起動 {{restartCount}} 回 · 前回起動 {{lastServiceStartAt}} · 前回終了 {{lastProcessDeathAt}}',
      androidKeepaliveRestart: '常駐サービスを再起動',
      androidKeepaliveSelfTest: '常駐セルフテストを実行',
      androidKeepaliveSelfTestKill: 'プロセス強制終了テスト',
      androidKeepaliveSelfTestKillConfirm: 'アプリのプロセスを即座に終了し、システムがサービスを再起動して LAN 復旧を検証します。ウィンドウが一時的に閉じますが続行しますか？',
      androidKeepaliveSelfTestRecovered: 'プロセスレベルテスト合格：プロセス終了後に LAN サービスが自動復旧（{{recoveredAt}}）',
      androidKeepaliveOpenNotification: '通知を許可',
      androidKeepaliveOpenBattery: 'バッテリー設定',
      androidKeepaliveRunning: '実行中',
      androidKeepaliveLanOnly: 'LANサーバーのみ',
      androidKeepaliveStopped: '停止 / 異常',
      ...
      androidOverlayTrigger: { background: 'バックグラウンド', keyboard: 'キーボード表示時', always: '常時', off: 'オフ' },
      androidOverlayTriggerHint: { background: 'シンプル', keyboard: 'このモードは保留中です。既存設定はバックグラウンドに戻します。', always: '常に表示', off: '音声入力ボタンを完全に非表示にします。単画素の常駐オーバーレイは個別に有効化できます。' },
