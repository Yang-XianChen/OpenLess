...
- **Android 端保活与 LAN 服务器**：为 Android 端加入保活能力（单像素悬浮窗保活等），并提供局域网远程听写服务器（`0.0.0.0:45678` WebSocket），供电脑端触发录音与听写。
- **电脑端极简客户端**：无界面 Linux remote-client，支持局域网自动发现、热键触发、切换/按住模式与 fcitx5 Wayland 通道，把手机回传的文本插入当前光标。
